...
void main( void) {
  ...
  vec2 uv = gl_FragCoord.xy; // OSC HELP
...
target = vec3(texture2D(osc_lfo, uv));


vec3 center_dir = (target - pos);
vec3 dir;
vec3 up = vec3(.0, 0., 1.); // OSC here would be nice
...
vec3 color = vec3(texture2D(osc_noise, uv) * 1.0);
 vec3 n = getnormal(ip);
if(dist < DIST_MIN){
	float diff = dot(l,n);
...
gl_FragColor = vec4(color, 1.0), texture2D(osc_lfo, uv);
}
